...
fn main_menu () {
    ...
    'main_loop: loop {
        println!(r#"Leave the choice empty to quit
...
3. Print base address"#);
        let menu_choice: u8 = loop { 
            ...
            Ok(num) => num,
            Err(error) => {
                if error.kind() == &IntErrorKind::Empty {break 'main_loop};
                println!("{error}");
                continue
                },
            };
        ...
        match menu_choice {
            ...
            // print base net
            3 => match base_net {
                Some(_) => display_net(&base_net.as_ref().unwrap()),
                None => println!("base net not initialized"),
            },

            _ => {println!("prazdne"); continue;},
        };
        continue
    };
}
...
pub fn create_subnets(base_network: &Net) -> Vec<Net> {
    ...
    let hosts_or_prefixes: u8 = loop { 
        std::io::stdin().read_line(&mut hosts_or_prefixes).expect("error");
...
fn create_base_net() -> Net {
    ...
    match mask_or_prefix {
        1 => is_mask = false,
        2 => is_mask = true,
        _ => {println!("Invalid number, defaulting to prefix."); is_mask = false},
    };
...
fn create_address() -> Address {
    ...
    println!("{}.{}.{}.{}", net_bytes[0], net_bytes[1], net_bytes[2], net_bytes[3]);
...
fn create_mask() -> Address {
    ...
    println!("{}.{}.{}.{}", mask_bytes[0], mask_bytes[1], mask_bytes[2], mask_bytes[3]);
